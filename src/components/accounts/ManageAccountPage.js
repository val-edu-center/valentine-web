...
const ManageAccountPage = ({ accounts, actions, allRoles, ...props }) => {
  ...
  const [account, setAccount] = useState(
    !!slug && accounts.length ? getUserById(accounts, slug) : createNewUser()
  );
  ...
  function changeRoles(account, role, checked) {
    ...
    if (role === "select-all") {
      const newRoles = checked
        ? allRoles
            .map((r) => r.getName())
            .filter((r) => !roleMapper.roleGroups.includes(r))
        : [];
      return {
        ...account,
  ...
  function formIsValid() {
    ...
    if (!username && enableUserAndPass)
      errors.username = "Username is required";
    if (!password && enableUserAndPass && !account.id)
  ...
  function handleSave(event) {
    ...
    const oldRoles = account.parseObject.get("roles")
      ? account.parseObject.get("roles")
      : [];
    const newRoles = account.roles;
    const rolesToAdd = allRoles
      .filter((r) => newRoles.includes(r.getName()))
      .filter((r) => !oldRoles.includes(r.getName()));
    const rolesToRemove = allRoles
      .filter((r) => oldRoles.includes(r.getName()))
      .filter((r) => !newRoles.includes(r.getName()));
    const newGroupRole =
      roleMapper.getGroupRole(account.parseObject.get("roles")) ===
      account.groupRole
        ? null
        : account.groupRole;
  ...
  return (
    <AccountForm
      allRoles={allRoles.map((r) => r.getName())}
