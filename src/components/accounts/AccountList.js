      ...
      <tbody>
        {users
          .filter(
            (user) =>
              !selectedGroup ||
              roleMapper.getGroupRole(user.roles) === selectedGroup
          )
          .filter((user) => isDeletable(user, session.roles))
          .map((user) => {
            return (
              <tr key={user.id}>
                <td>
...
const getGroupRoleInput = (user, isAdmin, onChange) => {
  ...
  if (groupRole) {
    if (isAdmin) {
      ...
      return (
        <select name="groups" id="group-select" value={groupRole} readOnly>
          {roleMapper.roleGroups.map((r) => getRoleOption(r))}
        </select>
      );
    }
...
function isDeletable(user, sessionRoles) {
  ...
  const isNotDeletable =
    roles.isDirector || (roles.isStaff && !sessionRoles.isDirector);
